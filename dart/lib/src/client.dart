...
class AirlinelookupValidationException implements Exception {
  final List<String> errors;

  AirlinelookupValidationException(this.errors);

  @override
  String toString() => 'AirlinelookupValidationException: ${errors.join("; ")}';
}
...
/// Airline Lookup API client.
///
/// For more information, visit: https://apiverve.com/marketplace/airlinelookup?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [iata] (required) - The IATA code of the airline for which you want to get the information (e.g., AA)
class AirlinelookupClient {
  final String apiKey;
  ...
  AirlinelookupClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/airlinelookup',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [AirlinelookupValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw AirlinelookupValidationException(errors);
    }
  }

  /// Execute a request to the Airline Lookup API.
  ///
  /// Parameters are validated before sending the request.
  Future<AirlinelookupResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw AirlinelookupException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return AirlinelookupResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw AirlinelookupException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw AirlinelookupException('Resource not found');
      } else {
        throw AirlinelookupException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is AirlinelookupException) rethrow;
      throw AirlinelookupException('Request failed: $e');
    }
  }
...
/// Exception thrown by the Airline Lookup API client.
class AirlinelookupException implements Exception {
  final String message;

  AirlinelookupException(this.message);

  @override
  String toString() => 'AirlinelookupException: $message';
}
